    ...
    <div>
      ...
      <div className="flex justify-center items-center gap-5 my-5 flex-wrap">
        {categories.map((cat,index) => (
          <button key={index} onClick={() => setSelectedCategory(cat)} className={`${selectedCategory == cat ? "bg-secondColor" : "bg-gray-300"} font-medium text-lg py-2 px-8 text-black rounded-xl shadow-md`}>{cat}</button>
        ))}
      </div>
      {/* products */}
      <div className="flex justify-center items-center gap-5 my-5 flex-wrap">
